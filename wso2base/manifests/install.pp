...
define wso2base::install ($mode, $install_dir, $pack_filename, $pack_dir, $owner, $group, $product_name) {
  ...
  case $mode {
    ...
    'file_bucket' : {
      ensure_resource('file', "$pack_file_abs_path", {
        ...
        owner       => $owner,
        group       => $group,
        source      => ["puppet:///modules/${product_name}/${pack_filename}", "puppet:///packs/${pack_filename}"],
        notify      => Exec["extract_${pack_file_abs_path}"],
        require     => File["${install_dir}", "${pack_dir}"]
      })
    }
  ...
  ensure_resource('exec', "extract_${pack_file_abs_path}", {
    ...
    timeout     => 0,
    refreshonly => true,
    notify      => File["${carbon_home}"]
  })

  # set ownership and file permissions for carbon_home
  ensure_resource('file', "${carbon_home}", {
    ensure      => 'directory',
    mode        => 754,
    owner       => $owner,
    group       => $group
