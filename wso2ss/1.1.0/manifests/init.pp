...
# Class: storage
#
# This class installs WSO2 Storage Server
#
# Parameters:
#  version            => '1.0.0',
#  offset             => 0,
...
class storage (
  ...
  $auto_scaler        = false,
  $auto_failover      = false,
  $securevault        = false,) inherits storage::params {
  ...
  storage::clean { $deployment_code:
    mode   => $maintenance_mode,
    target => $carbon_home,
  }

  storage::initialize { $deployment_code:
    repo      => $package_repo,
    version   => $version,
    mode      => $maintenance_mode,
  ...
  storage::deploy { $deployment_code:
    security => true,
    owner    => $owner,
    group    => $group,
    target   => $carbon_home,
  ...
  if $securevault {
    storage::push_templates { $securevault_templates:
      target    => $carbon_home,
      directory => 'wso2base',
      require   => Deploy[$deployment_code];
    }
