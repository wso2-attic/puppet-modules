...
class as (
  ...
  $group              = root,
  $target             = '/mnt',
  $monitoring         = false) inherits as::params {
  $deployment_code = 'as'
  $service_code = 'as'
  ...
  as::clean { $deployment_code:
    mode   => $maintenance_mode,
    target => $carbon_home;
  }

  as::initialize { $deployment_code:
    repo      => $package_repo,
  ...
  if $::securevault {
    as::push_templates { $securevault_templates:
      target    => $carbon_home,
      directory => 'wso2base',
      require   => Deploy[$deployment_code];
    }
