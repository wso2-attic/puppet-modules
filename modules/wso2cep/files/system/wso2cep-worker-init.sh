...
sed -i "/<eventSync>/,/<\/eventSync>/ s|<hostName>[0-9a-z.]\{1,\}</hostName>|<hostName>${local_ip}</hostName>|g"
echo "replaced eventSync/hostName with local ip ${local_ip}"

# replace management/hostName
sed -i "/<management>/,/<\/management>/ s|<hostName>[0-9a-z.]\{1,\}</hostName>|<hostName>${local_ip}</hostName>|g"
echo "replaced management/hostName with local ip ${local_ip}"

# replace presentation/hostName
sed -i "/<presentation>/,/<\/presentation>/ s|<hostName>[0-9a-z.]\{1,\}</hostName>|<hostName>${local_ip}</hostName>|g"
