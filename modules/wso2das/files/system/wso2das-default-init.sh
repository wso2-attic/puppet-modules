...
spark_defaults_conf_file_path=${server_path}/${server_name}/repository/conf/analytics/spark/spark-defaults.conf
...
# replace Spark driver bind hostname
sed -i "s/spark\.driver\.host.*/spark.driver.host ${local_ip}/g" ${spark_defaults_conf_file_path}
echo "replaced spark.driver.host with local ip ${local_ip}"

# Create symlink to CARBON_HOME which is needed for Spark
